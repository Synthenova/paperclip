...
export function buildUserMentionHref(userId: string, email?: string | null): string {
  const trimmedUserId = userId.trim();
  const normalizedEmail = normalizeUserEmail(email ?? null);
  if (!normalizedEmail) {
    return `${USER_MENTION_SCHEME}${trimmedUserId}`;
  }
  return `${USER_MENTION_SCHEME}${trimmedUserId}?e=${encodeURIComponent(normalizedEmail)}`;
}
...
export function parseUserMentionHref(href: string): ParsedUserMention | null {
  ...
  return {
    userId,
    email: normalizeUserEmail(url.searchParams.get("e") ?? url.searchParams.get("email")),
  };
}
...
function normalizeUserEmail(input: string | null | undefined): string | null {
  if (!input) return null;
  const trimmed = input.trim().toLowerCase();
  if (!trimmed || !USER_EMAIL_RE.test(trimmed)) return null;
  return trimmed;
}
